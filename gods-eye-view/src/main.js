...
async function init() {
  ...
  // Climate Eye Command Center Shell Mount (Step F4.1)
  if (typeof document !== 'undefined') {
    try {
      initClimateShell();
    } catch (err) {
      console.warn('[ClimateEye] Early shell mount warning:', err);
    }
  }
  ...
  try {
    ...
    loaderStatus.textContent = `Notice: 3D Globe fallback (${describeError(error)})`;
    loadingScreen.classList.add('hidden');
    if (typeof document !== 'undefined') {
      try {
        initClimateShell();
