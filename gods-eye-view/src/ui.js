...
import { initClimateShell } from './climate/panels/index.js';
import {
  cachedGroundFloor,
  cachedMeshFloor,
  GROUND_FLOOR_LIFT_M,
...
export class StyleManager {
  ...
  constructor(viewer, { mapStackController = null } = {}) {
    ...
    initDetection(viewer, [satellitesLayer], (modeLabel) => {
    ...
    this._applyClimateEyeUiRestrictions();
  ...
  _initUI() {
    ...
    this._globalKeydownHandler = (e) => {
      ...
      if (e.key.toLowerCase() === 'c') {
        // Dormant CCTV shortcut (disabled for Climate Eye)
      }
    };
  ...
  /**
   * Climate Eye Phase 2: Hides UI entry points and controls for removed GEV features.
   */
  _applyClimateEyeUiRestrictions() {
    // 1. Hide 3D aircraft model controls
    const models3dGroup = this._models3dBtn?.closest('.pp-toggle-group');
    if (models3dGroup) {
      models3dGroup.hidden = true;
      models3dGroup.style.display = 'none';
    }
    if (this._models3dBtn) {
      this._models3dBtn.hidden = true;
      this._models3dBtn.style.display = 'none';
    }
    if (this._models3dModeRow) {
      this._models3dModeRow.hidden = true;
      this._models3dModeRow.style.display = 'none';
    }

    // 2. Hide Radio companion dock & panel
    if (this._contextRadioDock) {
      this._contextRadioDock.hidden = true;
      this._contextRadioDock.style.display = 'none';
    }
    if (this._radioPanel) {
      this._radioPanel.hidden = true;
      this._radioPanel.style.display = 'none';
    }

    // 3. Hide CCTV panel & sync chip
    if (this._cctvPanel) {
      this._cctvPanel.hidden = true;
      this._cctvPanel.style.display = 'none';
    }
    const cctvChip = document.getElementById('cctv-sync-chip');
    if (cctvChip) {
      cctvChip.hidden = true;
      cctvChip.style.display = 'none';
    }

    // 4. Hide Traffic sync chip
    if (this._trafficSyncChip) {
      this._trafficSyncChip.hidden = true;
      this._trafficSyncChip.style.display = 'none';
    }

    // 5. Hide Contacts (Flights) and Space Missions entry points in context rail
    if (this._globalContextFlightsBtn) {
      this._globalContextFlightsBtn.hidden = true;
      this._globalContextFlightsBtn.style.display = 'none';
    }
    if (this._globalContextMissionsBtn) {
      this._globalContextMissionsBtn.hidden = true;
      this._globalContextMissionsBtn.style.display = 'none';
    }
    const contextModes = document.querySelector('.global-context-modes');
    if (contextModes) {
      contextModes.hidden = true;
      contextModes.style.display = 'none';
    }
    if (this._contextModeStandby) {
      this._contextModeStandby.hidden = true;
      this._contextModeStandby.style.display = 'none';
    }

    // 6. Hide Cockpit, Military Installations search, and TR-3B buttons
    const cockpitEntry = document.getElementById('cockpit-entry');
    if (cockpitEntry) {
      cockpitEntry.hidden = true;
      cockpitEntry.style.display = 'none';
    }
    if (this._installationsSearchBtn) {
      this._installationsSearchBtn.hidden = true;
      this._installationsSearchBtn.style.display = 'none';
    }
    if (this.tr3bToggle) {
      this.tr3bToggle.hidden = true;
      this.tr3bToggle.style.display = 'none';
    }

    // 7. Hide non-climate first-run choices (Contacts, Space Missions)
    const firstRunContacts = document.querySelector('[data-first-run-choice="contacts"]');
    if (firstRunContacts) {
      firstRunContacts.hidden = true;
      firstRunContacts.style.display = 'none';
    }
    const firstRunMissions = document.querySelector('[data-first-run-choice="space-missions"]');
    if (firstRunMissions) {
      firstRunMissions.hidden = true;
      firstRunMissions.style.display = 'none';
    }

    // 8. Hide legacy title bar so Climate Eye branding crowns the top
    const legacyTitleBar = document.getElementById('title-bar');
    if (legacyTitleBar) {
      legacyTitleBar.hidden = true;
    ...
    if (!this._climateShell && typeof document !== 'undefined') {
      try {
        this._climateShell = initClimateShell();
      } catch (err) {
        console.error('[ClimateEye] Failed to mount command-center shell:', err);
      }
    }
  }
